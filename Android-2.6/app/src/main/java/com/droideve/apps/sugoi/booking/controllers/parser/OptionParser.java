package com.droideve.apps.sugoi.booking.controllers.parser;
    ...
    public RealmList<Option> getOptions() {
        ...
        try {
            ...
            for (int i = 0; i < json.length(); i++) {
                ...
                if(json_options.has("parsed_quantity") && !json_options.isNull("parsed_quantity"))
                option.setParsed_quantity(json_options.getString("parsed_quantity"));

                if(json_options.has("parsed_time") && !json_options.isNull("parsed_time"))
                option.setParsed_time(json_options.getString("parsed_time"));

                if(json_options.has("time_post_start") && !json_options.isNull("time_post_start"))
                option.settimePost_Start(json_options.getString("time_post_start"));

                if(json_options.has("time_post_end") && !json_options.isNull("time_post_end"))
                option.settimePost_End(json_options.getString("time_post_end"));
